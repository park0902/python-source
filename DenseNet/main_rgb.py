...
from DenseNet.densenet_rgb import Model
import DenseNet.cifar10_pickle as cifar10
...
########################################################################################################################
## ▣ Data Training
##  - train data : 45,000 개 (10클래스, 클래스별 5,000개), validation data : 5,000 개, test data : 10,000 개
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for start_idx in range(0, len(train_data), batch_size):
            ...
            if epoch+1 in (150, 225):  # dynamic learning rate
        ...
        if validation_loss < best_loss_val:
            ...
            saver.save(sess, 'log/densenet_cifar10_v6.ckpt')
        else:
...
def restore_and_test():
    ...
    with tf.Session(config=config) as sess:
        ...
        sess.run(tf.global_variables_initializer())
        saver = tf.train.Saver()
        saver.restore(sess, 'log/densenet_cifar10_v6.ckpt')
