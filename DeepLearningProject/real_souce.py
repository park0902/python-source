    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_layer3') as scope:
                ...
                self.L3 = tf.nn.max_pool(value=self.L3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                         padding='SAME')  # 7x7 -> 4x4
            ...
            with tf.name_scope('fc_layer1') as scope:
                self.W5 = tf.get_variable(name='W5', shape=[4 * 4 * 256, 625], dtype=tf.float32,
                                          initializer=tf.contrib.layers.variance_scaling_initializer())
