...
with tf.Session() as sess:
    ...
    #     # epoch 별 끝난 시간 체크
    #     eetime = time.time()
    #     print('Epoch: ', '%04d' % (epoch + 1), 'cost =', avg_cost_list, 'epoch{} time ='.format(str(epoch+1)), round(eetime-sstime,6))
    ...
    ####################################################################################################################
        ...
        if epoch == 20:
            break
        eetime = time.time()
        print('Epoch: ', '%04d' % (epoch), 'cost =', avg_cost_list, ' - ', diff,', epoch{} time'.format(epoch),round(eetime-sstime,2),', ep_check',ep_check)
    print('Learning Finished!')
