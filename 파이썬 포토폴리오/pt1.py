...
class Visualization:
    ...
    def RKSI_2016_category(self):
        if question == 1:
            ...
            ax.set_title(u'월별 ' + title[category] + '그래프');ax.legend(loc='best')
            plt.xlabel(u'월');plt.ylabel(label[category])
    ...
    def RKSI_2016_summer_category(self):
        if question == 3:
            ...
            plt.title(u'여름 ' + title[category]);plt.xlabel(u'일');plt.ylabel(label[category]);plt.legend(loc=3)
    ...
    def RKSI_2016_winter_category(self):
        if question == 4:
            ...
            plt.title(u'겨울 ' + title[category]);plt.xlabel('Days');plt.ylabel(label[category]);plt.legend(loc=3)
    ...
    def RKSI_2016_bar_ver(self):
        if question == 6:
            ...
            plt.xlabel('일');
            plt.title(str(m) + ' 월 ' + title[category])
            ...
            if m == 1 or m == 3 or m == 5 or m == 7 or m == 8 or m == 10 or m == 12:
                ...
                for i, rect in enumerate(rects):
                    ax.text(rect.get_x() + rect.get_width() / 2.0, 0.95 * rect.get_height(), str(re[i]), ha='center')
                plt.ylabel(label[category]);plt.xlabel('일');plt.title(str(m) + ' 월 ' + title[category])
                plt.show()
    ...
    def Scatter(self):
        ...
        plt.scatter(re1, re2)
        plt.title(title[category1] + ' & ' + title[category2] + u' 산포도 ')
        plt.xlabel(label[category1]);plt.ylabel(label[category2])
    ...
    def Min_Dew_Vis(self):
        ...
        plt.plot(year, re, label='mean-dew', );plt.hold(True)
        plt.plot(year, vis, label='vis');plt.hold(False)
