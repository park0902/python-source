...
class ImageCrawler:
    def __init__(self):
        ...
        self.count = int(input('스크롤을 몇번 내릴까요?(한번당 15개 게시물 추가)  ')) + 1
        self.browser = None
        self.elem = None
        self.soup = None
    ...
    def Main(self):
        ...
        self.browser.quit()
        print(self.direc_name,'에 다운로드 완료했습니다.' )
        print('=========' * 2)
        print('크롤링이 완료되었습니다.')
        print('=========' * 2)
    ...
    def fetch_detail_url(self):
        self.direc_name = self.search_list['{}'.format(self.searchsite)][3]   # 디렉토리 확인해보고 없으면 만들어라!@!@!@!!!!!!
        if not os.path.isdir(self.direc_name):
            os.mkdir(self.direc_name)
...
class JournalCrawler:
    ...
    def Main(self):
        ...
        self.browser.quit()
        print(self.save_path, '에 다운로드 완료했습니다.')
        print('=========' * 2)
        print('크롤링이 완료되었습니다.')
        print('=========' * 2)
    ...
    def get_save_path(self):   #############################데이터 저장경로 지정################################
        self.save_path = 'c:\\data\\{}_중앙Texts.txt'.format(self.keyword)
        # save_path = save_path.replace("\\", "/") 필요 없을 것 같은데???

        if not os.path.isdir(os.path.split(self.save_path)[0]):
            os.mkdir(os.path.split(self.save_path)[0])  # 지정된 경로에 파일이 없으면 만들어라
...
class TwitCrawler():
    ...
    def get_save_path(self):
        self.save_path = self.FILE_PATH.replace("\\", "/")        # 굳이 할 필요 없음~
        if not os.path.isdir(os.path.split(self.save_path)[0]):   # 디렉토리 없으면 만들고~
    ...
    def get_twit_data(self):
        ...
        self.browser.get(self.crawURL)
        elem = self.browser.find_element_by_name("ands")
        since = self.browser.find_element_by_id("since")
        until = self.browser.find_element_by_id("until")
        ...
        time.sleep(5)
        html = self.browser.page_source  # 내가 브라우져로 보고있는 소스를 볼려고하는것이다.
...
class InstaCrawler:
    ...
    def fetch_detail_url(self):
        ...
        self.direc_name = self.FILE_PATH.replace("\\", "/")    # 디렉토리 확인해보고 없으면 만들어라!@!@!@!!!!!!
        if not os.path.isdir(self.direc_name):
            os.mkdir(self.direc_name)

        for idx ,p in enumerate(params, 1):
            # 다운받을 폴더경로 입력
            urllib.request.urlretrieve(p, self.direc_name + str(idx) + ".jpg")
