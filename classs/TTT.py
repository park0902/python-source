...
def main(cnt):                      # 메인모듈이 아니면 실행하지 마라!
        p1 = Human(1)               # Human 클래스를 p1 로 인스턴스화
        p2 = Human(2)               # Human 클래스를 p2 로 인스턴스화
        num = 0
        while num < cnt:                 # 무한루프 돌리겠다
            num += 1
            winner = play(p1, p2)   # play 함수가 돌다가 게임이 끝나면
            p1.episode_over(winner) # 1 or 2 or DRAW 를 리턴
