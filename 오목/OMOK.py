...
class Computer(object):
    def __init__(self, player):     # 컴퓨터가 두번째 두면 player 에 2가 담긴다
        self.player = player
    ...
    def random(self, state):
        ...
        for i in range(9):
            for j in range(9):
                if state[i][j] == EMPTY:
                    available.append((i, j))    # [(0,0),(0,1),(0,2)....]
        return random.choice(available)         # available에서 비어있는 것 중에 하나를 랜덤 선택
